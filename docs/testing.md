...
```bash
python -m pytest -q            # full suite (64 tests)
...
| `tests/test_backend/test_brain.py` | `BrainService`: prompt validation, tool routing without LLM (time/system), pending-action confirmation flow, LLM streaming with a mocked httpx client, Ollama HTTP error surfacing, `ChatRequest` model limits (empty + 50 001-char prompts) |

Suite status on the reference machine (Windows, Python 3.14.6): **64
passed**, 1 expected `StarletteDeprecationWarning` (httpx vs httpx2 in
starlette's TestClient — harmless), ruff clean.

## Conventions

- **No live services in tests.** No Ollama, no real `nvidia-smi`, no real
