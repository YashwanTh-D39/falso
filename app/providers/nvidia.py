...
class NVIDIAProvider(BaseAIProvider):
    ...
    def client(self) -> httpx.AsyncClient:
        ...
        if self._client is None or self._client.is_closed:
            ...
            timeout = httpx.Timeout(
                settings.ai_timeout_seconds, connect=10.0, read=45.0
            )
    ...
    async def verify_model_availability(self) -> bool:
        """Check if the configured model exists in NVIDIA's /models catalog."""
        try:
            ...
            if resp.status_code == 200:
                ...
                if self.model in model_ids:
                    logger.info("NVIDIA model verified in API catalog: %s", self.model)
                    return True
                else:
                    logger.warning(
